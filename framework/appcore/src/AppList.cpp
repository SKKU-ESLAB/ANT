...
void AppList::initializeDefaultApps(std::string systemAppsDir) {
  int systemAppNamesCount = 5;
  const char *systemAppNames[] = {"CameraViewer", "SensorViewer",
  ...
  for (int i = 0; i < systemAppNamesCount; i++) {
    ...
    snprintf(packagePath, PATH_BUFFER_SIZE, "%s/%s", systemAppsDir.c_str(),
             systemAppNames[i]);
    snprintf(mainJSFileName, PATH_BUFFER_SIZE, "%s", "index.js");
    snprintf(iconFileName, PATH_BUFFER_SIZE, ".");
    App *app = new App(this->mNextAppId++, true, systemAppNames[i], packagePath,
                       mainJSFileName, iconFileName, AppState::Ready);
    ANT_DBG_VERB("Add %s: %s, %s", systemAppNames[i], packagePath,
                 mainJSFileName);
    this->add(app);
    this->flush(app);
  }
...
bool AppList::fetchAppList() {
  ...
  int prepareRes = sqlite3_prepare_v2(this->mDB, query, -1, &stmt, NULL);
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    app = new App(appId, isDefaultApp, name, packagePath, mainJSFileName,
                  iconFileName, AppState::Ready);
...
App *AppList::getByAppId(int appId) {
  ...
  for (alIter = this->mApps.begin(); alIter != this->mApps.end(); ++alIter) {
...
bool AppList::flush(App *app) {
  ...
  snprintf(query, QUERY_BUFFER_LENGTH,
           ...
           "VALUES ('%d', '%d', '%s', '%s', '%s', '%s');",
           appId, ((isDefaultApp) ? 1 : 0), name.c_str(), packagePath.c_str(),
           mainJSFileName.c_str(), iconFileName.c_str());
...
bool AppList::remove(App *app) {
  ...
  // Remove from DB
  char query[QUERY_BUFFER_LENGTH];
  snprintf(query, QUERY_BUFFER_LENGTH, "DELETE FROM APPLIST WHERE ID=%d;",
           appId);
  char *insertErr;
  int insertRes = sqlite3_exec(this->mDB, query, NULL, NULL, &insertErr);
  if (insertRes != SQLITE_OK) {

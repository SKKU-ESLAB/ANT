...
class SensorReader
{
	...
	public:
		SensorReader(){
			...
			fd = open("sensor/lumo/posture", O_RDWR | O_NONBLOCK );
...
public:
    ...
    OduinoResource()
    {
        ...
        m_name = "ESLab lumo posture Sensor";
        m_power = 0;
        m_oduinoUri = "/lumo/posture";
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("posture", tempString)){
		m_power = atoi(tempString.c_str());
                cout << "\t\t\t\t" << "posture:  " << m_power << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "posture not found in the representation" << endl;
            }
        }
        catch (exception& e)
        {

...
class SensorReader
{
	...
	public:
		SensorReader(){
			...
			fd = open("sensor/kitchen/infrared", O_RDWR | O_NONBLOCK );
...
public:
    ...
    OduinoResource()
    {
        ...
        m_name = "ESLab Infrared Sensor";
        m_power = 0;
        m_oduinoUri = "/kitchen/infrared";
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("lux", tempString))
            {
		m_power = atoi(tempString.c_str());
                cout << "\t\t\t\t" << "lux:  " << m_power << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "lux not found in the representation" << endl;
            }
        }
        catch (exception& e)
        {

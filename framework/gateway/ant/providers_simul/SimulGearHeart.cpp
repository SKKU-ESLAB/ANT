...
class SensorReader
{
	...
	public:
		SensorReader(){
			...
			fd = open("sensor/gear/heart", O_RDWR | O_NONBLOCK );
...
public:
    ...
    OduinoResource()
    {
        ...
        m_name = "ESLab Heartbeat Sensor";
        m_power = 0;
        m_oduinoUri = "/gear/heart";
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("bpm", tempString)){
		m_power = atoi(tempString.c_str());
                cout << "\t\t\t\t" << "bpm:  " << m_power << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "bpm not found in the representation" << endl;
            }
        }
        catch (exception& e)
        {

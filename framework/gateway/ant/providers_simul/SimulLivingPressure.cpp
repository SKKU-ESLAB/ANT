...
class SensorReader
{
	...
	public:
		SensorReader(){
			...
			fd = open("sensor/living/pressure", O_RDWR | O_NONBLOCK );
...
public:
    ...
    OduinoResource()
    {
        ...
        m_name = "ESLab Pressure Sensor";
        m_power = 0;
        m_oduinoUri = "/living/pressure";
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("pressure", tempString))
            {
		m_power = atoi(tempString.c_str());
                cout << "\t\t\t\t" << "pressure:  " << m_power << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "pressure not found in the representation" << endl;
            }
        }
        catch (exception& e)
        {
...
*/
		...
		if((double)abs(curPower-prevPower) > (double)prevPower / 1000 * (level * 100 / 255)){
		    prevPower = curPower;
		    return true;
		}
		else{

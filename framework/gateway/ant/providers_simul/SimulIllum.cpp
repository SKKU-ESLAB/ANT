...
class SensorReader
{
	private:
		int fd,c,res;
		int uv, visible, infrared;
		char buf[ODU_BUF_SIZ];
	public:
		SensorReader(){
			struct termios toptions;

			fd = open("sensor/living/illum", O_RDWR | O_NONBLOCK );
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("infrared", tempString)){
		m_infrared = atoi(tempString.c_str());
		cout << "\t\t\t\t" << "infrared:  " << m_infrared << endl;
	    }
	    else
	    {
		cout << "\t\t\t\t" << "infrared not found in the representation" << endl;
	    }
        }
        catch (exception& e)
        {
    ...
    void get(OCRepresentation &rep)
    {
       if(sr->listen()){
		...
		m_infrared = sr->get_Infrared();
        }
        std::stringstream ss, ss1, ss2;
        ss << m_uv;
        rep.setValue("uv", ss.str());
	ss1 << m_visible;
	rep.setValue("visible", ss.str());
	ss2 << m_infrared;
	rep.setValue("infrared", ss.str());
    }
...
*/
		...
		net_level = level;
		int curUv, curVisible, curInfrared;
		curUv = OduinoResource::getInstance()->getUv();
		curVisible = OduinoResource::getInstance()->getVisible();
		curInfrared = OduinoResource::getInstance()->getInfrared();
//		printf("temp:%d/%d\thumi:%d/%d\n", curTemp,prevTemp,curHumi,prevHumi);
		if((double)abs(curUv-prevUv) > (double)prevUv / 1000 * (level * 100 / 255)){
		    prevUv = curUv;
		    prevVisible = curVisible;
		    prevInfrared = curInfrared;
		    return true;
		}
		...
		else if((double)abs(curInfrared-prevInfrared) > (double)prevInfrared / 1000 * (level * 100 / 255)){
		    prevUv = curUv;
		    prevVisible = curVisible;
		    prevInfrared = curInfrared;
		    return true;

...
class SensorReader
{
	...
	public:
		SensorReader(){
			...
			fd = open("sensor/living/damp1", O_RDWR | O_NONBLOCK );
...
public:
    ...
    OduinoResource()
    {
        ...
        m_name = "ESLab Damp Sensor";
        m_power = 0;
        m_oduinoUri = "/living/damp1";
...
public:
    ...
    void put(OCRepresentation& rep)
    {
	    ...
	    if (rep.getValue("damp", tempString))
            {
		m_power = atoi(tempString.c_str());
                cout << "\t\t\t\t" << "damp:  " << m_power << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "damp not found in the representation" << endl;
            }
        }
        catch (exception& e)
        {

...
public:
    ...
    OCRepresentation m_oduinoRep;
    ObservationIds m_interestedObservers;

public:
    /// Constructor
    OduinoResource()
        :m_name("ESLab Oduino"), m_temp(0), m_humi(0), m_oduinoUri("/a/oduino") {
        // Initialize representation
        std::stringstream ss1;
        std::stringstream ss2;
        std::stringstream ss3;
        m_oduinoRep.setUri(m_oduinoUri);

        ss1<<m_temp;
        m_oduinoRep.setValue("temp", ss1.str());
        ss2<<m_humi;
        m_oduinoRep.setValue("humi", ss2.str());
        ss3<<m_name;
        m_oduinoRep.setValue("name", ss3.str());
    }
    ...
    void createResource()
    {
        std::string resourceURI = m_oduinoUri; //URI of the resource
        std::string resourceTypeName = "core.provider"; //resource type name. In this case, it is light
        ...
        EntityHandler cb = std::bind(&OduinoResource::entityHandler, this,PH::_1);
	...
	pthread_t sr_threadId;
	static int sr_startedThread = 0;
	if(!sr_startedThread)
            {
                pthread_create (&sr_threadId, NULL, ReadOduinoData, (void *)this);
                sr_startedThread = 1;
            }
    }
    ...
    {
        std::string resourceURI = "/a/oduino1"; // URI of the resource
        std::string resourceTypeName = "core.provider"; // resource type name. In this case, it is light
        std::string resourceInterface = DEFAULT_INTERFACE; // resource interface.
    ...
    {
        try {
		cout << "getJSONRepresentation : " << rep.getJSONRepresentation() <<endl;
    ...
    OCRepresentation post(OCRepresentation& rep)
    {
        ...
        if(first)
        {
            ...
            if(OC_STACK_OK == createResource1())
            {
                ...
                rep1.setValue("createduri", std::string("/a/oduino1"));
    ...
    OCRepresentation get()
    {
        ...
        ss << m_temp;
        m_oduinoRep.setValue("temp", ss.str());
        ss1 << m_humi;
        m_oduinoRep.setValue("humi", ss1.str());
...
OCEntityHandlerResult entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    ...
    if(request)
    {
        ...
        if(requestFlag & RequestHandlerFlag::ObserverFlag)
        {
            ...
            if(!startedThread)
            {
                pthread_create (&threadId, NULL, ChangeOduinoRepresentation, (void *)this);
                startedThread = 1;
            }
            ehResult = OC_EH_OK;
        }
...
void * ReadOduinoData (void *param){
	...
	OduinoResource* oduinoPtr = (OduinoResource *) param;

	while(1){
		sleep(2);
		if(sr->listen()){
		oduinoPtr->m_temp = sr->get_Temp();
		oduinoPtr->m_humi = sr->get_Humi();
#ifdef SERIALPORTDEBUG
...
void * ChangeOduinoRepresentation (void *param)
{
    ...
    while (1)
    {
	    ...
	    int subtraction = prev_temp - oduinoPtr->m_temp;
            if(1 < subtraction || subtraction < -1){
		    cout << "\nTemperature updated to : " << oduinoPtr->m_temp << endl;
		    cout << "Notifying observers with resource handle: " << oduinoPtr->getHandle() << endl;
		    prev_temp = oduinoPtr->m_temp;
		    ...
		    if(isListOfObservers)
		    {
			    ...
			    resourceResponse->setResourceRepresentation(oduinoPtr->get(), DEFAULT_INTERFACE);

			    result = OCPlatform::notifyListOfObservers(  oduinoPtr->getHandle(),
					    oduinoPtr->m_interestedObservers,
					    resourceResponse);
		    }
		    else
		    {
			    result = OCPlatform::notifyAllObservers(oduinoPtr->getHandle());
		    }
        ...
        {subtraction = prev_humi - oduinoPtr->m_humi;
	    if(1 < subtraction || subtraction < -1){
		    cout << "\nHumidity updated to : " << oduinoPtr->m_humi << endl;
		    cout << "Notifying observers with resource handle: " << oduinoPtr->getHandle() << endl;
		    prev_humi = oduinoPtr->m_humi;
...
void * handleSlowResponse (void *param, std::shared_ptr<OCResourceRequest> pRequest)
{
    ...
    pResponse->setResourceRepresentation(oduinoPtr->get());
    pResponse->setErrorCode(200);
    pResponse->setResponseResult(OC_EH_OK);
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        OduinoResource myOduino;

        // Invoke createResource function of class light.
        myOduino.createResource();

        myOduino.addType(std::string("core.specialoduino"));
        //
        myOduino.addInterface(std::string("oc.mi.so"));

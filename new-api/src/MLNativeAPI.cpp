...
// Static Constructor
Persistent<Function> MLNativeAPI::sConstructor;

#define JS_THIS_OBJECT_NAME "MLNativeAPI"

/* 
 * Prototype Initializer
 */
void MLNativeAPI::InitPrototype(Isolate* isolate) {
 ...
 * Constructor 1
 */
void MLNativeAPI::NewInstance(const FunctionCallbackInfo<Value>& args) {
 ...
 * Constructor 2
 */
void MLNativeAPI::New(const FunctionCallbackInfo<Value>& args) {
  Isolate* isolate = args.GetIsolate();
  ...
  if(args.IsConstructCall()) {
    ...
    MLNativeAPI* newObject = new MLNativeAPI();
 ...
 */
void MLNativeAPI::RunModel(const FunctionCallbackInfo<Value>& args) {

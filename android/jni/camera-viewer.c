...
static void gst_native_init (JNIEnv* env, jobject thiz, jstring addr, jstring strport) {
  ...
  GST_DEBUG_CATEGORY_INIT (debug_category, "camera-viewer", 0, "Camera Viewer");
  gst_debug_set_threshold_for_name("camera-viewer", GST_LEVEL_DEBUG);
...
static jboolean gst_native_class_init (JNIEnv* env, jclass klass) {
  ...
  if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id ||
     ...
     */
    __android_log_print (ANDROID_LOG_ERROR, "camera-viewer", "The calling class does not implement all necessary interface methods");
    return JNI_FALSE;
  }
  return JNI_TRUE;
}
...
static void gst_native_surface_init (JNIEnv *env, jobject thiz, jobject surface) {
  ...
  //gst_video_overlay_prepare_window_handle (GST_VIDEO_OVERLAY (data->video_sink));
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  if ((*vm)->GetEnv(vm, (void**) &env, JNI_VERSION_1_4) != JNI_OK) {
    __android_log_print (ANDROID_LOG_ERROR, "camera-viewer", "Could not retrieve JNIEnv");
    return 0;
  }
  jclass klass = (*env)->FindClass (env, "com/ant/ant_manager/view/CameraViewerActivity");

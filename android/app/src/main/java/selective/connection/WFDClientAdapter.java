...
public class WFDClientAdapter extends NetworkAdapter {
    ...
    public boolean make_connection() {
        ...
        if (dev_addr == null) {
            try {
                ...
                if (dev_addr != null){
                    Logger.print(tag, "Server up");
                    res = true;
                } else {
                    Logger.print(tag, "Waited");
                    res = false;
                }
        ...
        mManager.discoverPeers(mChannel, new WifiP2pManager.ActionListener() {
            ...
            public void onSuccess() {
                Logger.print(tag, "Discover peers succeeded");

            }

            @Override
            public void onFailure(int reason) {
                Logger.print(tag, "Discover peers failed");
            }
        });
        ...
        try {
            ...
            Logger.print(tag, "Connect to " + ip);
            sock = new Socket();
            sock.bind(null);
    ...
    public void on_control_recv(byte[] buf, int len) {
        if (len == 17) {
            ...
            Logger.print(tag, dev_addr + " & " + wps_pin);
            synchronized(server_up_trigger) {

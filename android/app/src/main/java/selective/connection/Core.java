        ...
        public void run() {
            ...
            while (true) {
                ...
                if (ctrl_req == kCtrlReqIncr) {
                    ...
                    Core.get_instance().prev_state = Core.get_instance().state;
                    Core.get_instance().state = kNetStatIncr;
                    ...
                    Core.get_instance().prev_state = Core.get_instance().state;
                    Core.get_instance().state = kNetStatDecr;
        ...
        mCtrlHandler = new Handler() {
            public void handleMessage(Message msg) {
                if (msg.what == kNetStatDiscon) {
                    Core.get_instance().connect_control_adapter();
                }
            }
        };
    }
        ...
        na.connect(new Handler() {
            public void handleMessage(Message msg) {
                if (msg.what == kDevCon) {
                    ...
                    Core.get_instance().state = kNetStatControl;
                    Core.get_instance().recv_thread = new CtrlRecvThread(mCtrlHandler);
                    Core.get_instance().recv_thread.start();
                } else {
                    Core.get_instance().state = kNetStatDiscon;
                    try {
                        Thread.sleep(500);
